...
class LottoScraper:
    ...
    def scrape_latest_results(self) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                if "csv" in content_type or "text/csv" in content_type:
                    ...
                    logger.info(
                        "Direct download did not return CSV, proceeding with Selenium"
                    )
            except Exception as e:
            ...
            try:
                ...
                for btn in buttons:
                    ...
                    if btn_text and (
                        "תוצאות" in btn_text
                        or "results" in btn_text.lower()
                        or "הגרלה" in btn_text
                    ):
                        btn.click()
            ...
            result_elements = soup.find_all(
                ["div", "li", "span", "table"],
                class_=re.compile(r".*lotto.*|.*result.*|.*draw.*|.*number.*", re.I),
            )
        ...
        try:
            ...
            for elem in elements:
                ...
                if "הגרלה" in text or "Draw" in text or "מספר" in text:
                    match = re.search(
                        r"\d{3,5}", text
                    )  # Broader range for draw numbers
                    if match:
                        if current_draw and len(numbers) >= 6:
                            ...
                            logger.info(
                                f"Saved draw {current_draw} with {len(numbers)} numbers"
                            )
                ...
                if "חזק" in text or "strong" in text.lower():
                    ...
                    if strong_match:
                        ...
                        logger.debug(
                            f"Found strong number {strong_number} for draw {current_draw}"
                        )
                    else:
                        ...
                        if next_elem:
                            ...
                            if strong_match:
                                strong_number = strong_match.group()
                                logger.debug(
                                    f"Found strong number {strong_number} in next element for draw {current_draw}"
                                )
            ...
            if current_draw and len(numbers) >= 6:
                ...
                logger.info(
                    f"Saved last draw {current_draw} with {len(numbers)} numbers"
                )
    ...
    def update_data_file(self) -> bool:
            ...
            if os.path.exists(self.csv_path):
                try:
                    ...
                    combined = combined.drop_duplicates(
                        subset=["draw_number"], keep="first"
                    )

                    # Sort by draw_number (descending for newest first)
                    combined["draw_number"] = pd.to_numeric(
                        combined["draw_number"], errors="coerce"
                    )
